...
`.github/workflows/release-lock-update.yml` (`workflow_dispatch`) is the
reviewed path for actually moving a pin: it regenerates the lock and opens a
PR against `dev`, which then exercises the new lock through
`clean-host.yml`'s full four-platform freeze-and-conformance matrix before it
ever reaches a tag.
