...
#: The one backend whose app dir the planner is allowed to SUBSTITUTE.
#: `_zephyr_app_dir`'s parent fallback is reached only from the `os: zephyr`
#: branch of `tan/planner/orchestrator.py`; `os: baremetal` assembles
#: `cmake -S <app>` from `_resolve_app_path` with no `CMakeLists.txt` probe
#: and therefore no substitution to announce, so folding it in here would
...
def _substituted_app_dirs(plan: BuildPlan, build_root: Path) -> dict[int, str]:
    """Slice indices whose `cores.<id>.app` EXISTS but carries no
    `CMakeLists.txt`, so the planner silently built its PARENT instead
    (tan-cli#517), each mapped to the message announcing it.

    `tan/planner/orchestrator.py`'s `_zephyr_app_dir` resolves `app:`, probes
    `(p / "CMakeLists.txt").is_file()`, and on failure falls back to
    `(p.parent / "CMakeLists.txt").is_file()` -- naming neither path
    anywhere. On a multi-core project that parent is the project root, which
    ...
    The fallback is NOT removed and is NOT refused. It is the shipped
    convention: 96 of 105 enabled zephyr/baremetal core entries across
    alp-sdk `dev`'s own examples are `app: ./src`, a sources-only directory
    whose `CMakeLists.txt` lives one level up. Refusing it would break 91%
    of the SDK's own examples, which is why tan-cli#517 asks for the
    substitution to be ANNOUNCED rather than stopped.

    Checked HERE rather than in `tan/planner/**` for the same reason
    `_missing_app_dirs` above is: the planner is a hash-audited relocation of
    alp-sdk's `scripts/alp_orchestrate/**` (`test_planner_relocation_
    freshness.py`), so a warning added there would diverge this repo's copy
    in SHAPE from upstream and go red on the next re-hash. The condition is
    therefore MIRRORED from `_zephyr_app_dir`, not imported -- if that
    function's fallback ever changes, the freshness gate fails first and
    names the file, which is the signal to re-mirror this.

    Read off the plan's own `appDir`, which is the CONFIGURED path: the
    planner emits `appDir` from `_resolve_app_path` (`buildplan.py`) and
    hands the SUBSTITUTED directory only to the slice's `west build`
    positional. So the plan already carries both halves and this recovers
    the pair without parsing an argv position.

    Same three filters `_missing_app_dirs` uses, and anchored on
    `build_root` the same way: a slice the planner never emitted a command
    for was never going to build anything, and a `yocto` slice dispatches by
    `recipe:` rather than by this directory at all."""
    ...
    for i, sl in enumerate(plan.slices):
        ...
        if not (parent / "CMakeLists.txt").is_file():
            # Neither has one: `_zephyr_app_dir` returns the configured path
            # unchanged, so nothing was substituted. The build still fails,
            # deeper in, with its own message -- inventing a substitution
            # here would name a parent the planner never chose.
            continue
        substituted[i] = (
            f"core `{sl.core_id}`: app directory has no CMakeLists.txt "
            f"({app_dir_path}) -- building its parent instead ({parent}). This is the "
            f"shipped `app: ./src` convention; if that parent is not this core's "
            f"application, `cores.{sl.core_id}.app` names the wrong directory."
        )
    return substituted
