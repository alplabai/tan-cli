...
#: `alp_project.py`'s own `_write_or_print` announces every write it makes on
#: stderr, unconditionally, even a zero-byte one: `alp_project: wrote <path>
#: (<n> bytes)`, THEN returns 0. tan-cli#498: an unscoped per-core target
#: (`zephyr-conf`, `cmake-args` measured) legitimately writes an empty file
#: and exits 0 when no core in this project's `cores:` block matches the
#: mode's OS class -- `_missing_emit_output` used to read that zero-byte file
#: as proof of the tan-cli#397 failure it exists to catch, so a correct,
#: SDK-confirmed empty emit was reported `generate.emit-failed` (and, with
#: `--output`, its file then UNLINKED by `_discard_probe_file`). Anchored on
#: the literal prefix `_write_or_print` always uses, not on the byte count
#: alone -- matching on count alone would swallow #397's own regression, a
#: silent write that produced nothing and said nothing.
_SDK_CONFIRMED_EMPTY_WRITE_RE = re.compile(r"^alp_project: wrote .* \(0 bytes\)\s*$", re.MULTILINE)


def _missing_emit_output(emit: str, output: Path, stderr: str) -> str | None:
    ...
    NON-EMPTY is no longer the whole rule (tan-cli#498 corrected "no real emit is empty" -- false
    for `zephyr-conf`/`cmake-args` on a project touching no core of the mode's OS class). A present,
    zero-byte `output` is accepted ONLY when `stderr` carries the SDK's own `_write_or_print`
    confirmation of that exact write -- see `_SDK_CONFIRMED_EMPTY_WRITE_RE`. Otherwise a zero-byte
    file is still treated as before: `_ensure_writable` proves writability with an `open("ab")` that
    CREATES one (and a `mkdir` for `zephyr-board`'s directory), so existence alone would accept
    tan's own probe as the artefact -- `--output` is the shape `cmake/alp.cmake` drives for all 96
    Zephyr examples.
    ...
    try:
        ...
        if output.stat().st_size == 0 and not _SDK_CONFIRMED_EMPTY_WRITE_RE.search(stderr):
            return prefix + f"did not write `{output}`."
    except (OSError, ValueError) as err:
...
def _emit_one(
    ...
    if out.returncode == 0:
        return _missing_emit_output(emit, output, out.stderr or "")
