...
Scope (issue #523, first slice; extended by #655 slice 1 and alp-sdk#1964,
both hand-ported here as tan-cli#1156):
  ...
  - Renesas RZ/V2N-family boards (`v2n` / `v2n-m1`, e.g. E1M-V2N101,
    ...
    (silicon_peripheral, silicon_pad) -- alp-sdk's `scripts/
    validate_metadata.py` cross-checks every row resolves to exactly one
    `owner: "renesas"` pad there -- rather than re-declaring pad<->signal
    authority a second time.

    The board `.dts` is ALSO generated as of #655 slice 2 (alp-sdk
    `e4f2f60e4`): `emit_zephyr_board()`'s `v2n`/`v2n-m1` branch calls
    `_v2n_dts()`, which reuses the upstream RZ/V2N SoC devicetree and adds
    the same on-module GD32G553 supervisor links `_v2n_pinctrl_dtsi()`
    reads. A SoM preset's `topology.m33_sm.openamp_ipc` (boolean, absent
    means false) gates the OpenAMP/MHU-B reserved-memory block and the
    CAN-FD-unavailable analysis -- true only for E1M-V2N101 today;
    E1M-V2M101's currently-committed tree carries neither block, and this
    flag lets the generator reproduce that gap rather than inventing
    content for a board nobody wrote it for. `Kconfig.defconfig` remains
    the one hand-authored file in this family (no metadata source for it
    yet).
...
def _aen_brd_i2c_dts(links: dict[str, Any], part: str) -> list[str]:
    ...
    *part* (the SoC JSON's `part`, e.g. ``"E8"``, ``"E3"``) gates the
    ...
    RELOCATED divergence, RETIRED (alp-sdk#1988 / tan-cli#493): this used to
    gate on a hardcoded ``if part == "E8"`` against a plain-string
    `on_module_links.rtc_alarm.risk` (tan's own independent fix for the same
    hazard `_aen_brd_i2c_dts` names -- an E8-only DFP warning bleeding into
    an E3/E4/E6 board tree). alp-sdk's own fix for the same issue landed
    later and reshapes the metadata field itself into a per-part map
    (``risk: {E8: "..."}``) rather than gating in code -- ported here to
    match: an unstated part gets no paragraph, and any future DFP that
    confirms the same quirk is added to the map, not to a code branch.
    """
    bus = links["brd_i2c"]
    alarm = links["rtc_alarm"]
    sda = _pin_by_peripheral(bus["pins"], "I2C0_SDA_C")
    ...
    # per-part `risk:` map (alp-sdk#1988) -- evidenced only against this
    # board's own SoC part's DFP: on-module-links.yaml's `risk:` names only
    # "The AE822 DFP" -- the E8 part's datasheet -- so a family-scoped
    # emission bled E8 register-layout guidance into an E1M-AEN301 (E3)
    # board tree (tan-cli#493's
    # test_a_non_e8_aen_sku_includes_its_own_peripherals_overlay caught it).
    # A part with no entry in the map gets no paragraph: an unstated DFP
    # citation is not evidence for this silicon.
...
def emit_zephyr_board(
    ...
    *relative_path* is rooted at the board directory (e.g.
    ...
    if family == "aen":
        ...
        # `Kconfig.defconfig` stays hand-authored in this slice (it does
        # not exist in the V2N/V2M board directories); the board `.dts`
        # does not -- #655 slice 2 closed that gap (alp-sdk e4f2f60e4).
        # Which families this branch covers is read from
        ...
        # into all three files' banners too (below), not just the
        # family-agnostic ones.
        banner_extra_source.update(dict.fromkeys(
