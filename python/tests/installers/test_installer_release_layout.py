...
#: `else` arm (`payload="$tmp"`, install.sh:434) that reuses an already-trapped
#: temp path -- so it can never catch a leak in the archive arm's OWN
#: `payload="$(mktemp)"` launcher file (install.sh:424), which is exactly what
#: this tag exists to reach.
...
def test_sh_bad_archive_payload_leaves_no_launcher_temp_file(release_server, tmp_path):
    """tan-cli#803: the archive-layout launcher (`$payload`, install.sh:424 --
    the "Generated by tan install.sh (tan-cli#349)" heredoc) is allocated by
    its OWN `mktemp` call, made well AFTER `trap ... EXIT` (install.sh:240) was
    installed, and was never added to it. `BAD_PAYLOAD_TAG` cannot catch this:
    its assets are RAW-shaped, so install.sh takes the `else` arm
    (`payload="$tmp"`, install.sh:434) that reuses a path the trap already
    covers -- `BAD_PAYLOAD_ARCHIVE_TAG` is the archive-shaped counterpart this
    leak actually needs to be reached at all.
...
def test_sh_rc_update_is_skipped_gracefully_when_home_is_unset(release_server, tmp_path):
    ...
    block picks (install.sh:876-930) is `$HOME/<dotfile>`. Under `set -eu`,
    dereferencing `$HOME` while it is unset would abort the script right there
    -- again, well past the point the install already committed.
