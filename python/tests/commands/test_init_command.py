...
# ---------------------------------------------------------------------------
# tan-cli#495 blocker 2 -- `--som` over-refusal on a family-split template
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("template_id", ["zephyr-app", "sensor-starter", "board-diagnostics"])
@pytest.mark.parametrize("sku", ["E1M-AEN301", "E1M-AEN401", "E1M-AEN701"])
def test_a_non_a32_alif_sku_is_accepted_by_a_template_whose_tree_never_declares_a32(
    template_id, sku, tmp_path
):
    """These three templates' Alif tree declares only `m55_hp` -- the family's
    OWN guaranteed baseline core -- so they are correct for the WHOLE Alif
    family, not just E1M-AEN801. The fix this replaces applied ONE blanket
    family table (E1M-AEN801 only) to every family-split template uniformly,
    refusing this exact combination even though it previously worked and
    `tan validate` would accept it."""
    proc = run_tan(
        "init", "--template", template_id, "--som", sku, "--format", "json", cwd=tmp_path
    )
    env = envelope(proc)

    assert proc.returncode == 0, env["issues"]
    board = (tmp_path / "board.yaml").read_text(encoding="utf-8")
    assert f"sku: {sku}" in board
    assert "a32_cluster" not in board


def test_edge_ai_starter_still_refuses_an_alif_sku_with_no_a32_cluster(tmp_path):
    """Contrast with the sibling test above: `edge-ai-starter`'s Alif tree DOES
    pre-declare `a32_cluster` (present on E1M-AEN801/Ensemble E8 alone), so it
    keeps refusing every other Alif SKU -- the fix must not overcorrect into
    accepting a combination that is genuinely wrong."""
    proc = run_tan(
        "init", "--template", "edge-ai-starter", "--som", "E1M-AEN301",
...
@pytest.mark.parametrize("sku", ["E1M-V2N101", "E1M-V2N102", "E1M-V2M101", "E1M-V2M102"])
def test_edge_ai_starter_still_accepts_every_renesas_family_sku(sku, tmp_path):
    """No regression check: the Renesas bucket was already correct (all four
    SKUs share one PCB/silicon variant and genuinely carry the `a55_cluster`
    `edge-ai-starter`'s Renesas tree pre-declares) -- the fix that widened the
    Alif side must not narrow this one."""
    proc = run_tan(
        "init", "--template", "edge-ai-starter", "--som", sku, "--format", "json", cwd=tmp_path
    )
    env = envelope(proc)

    assert proc.returncode == 0, env["issues"]
    assert f"sku: {sku}" in (tmp_path / "board.yaml").read_text(encoding="utf-8")


def test_a_resolved_sdk_checkout_overrides_the_static_som_refusal_via_real_topology(tmp_path):
    """tan-cli#494 defect 2 follow-up: a customer with a legitimate, just-added
    SKU must not be told their hardware is unsupported when a checkout is
    available to PROVE it is. `E1M-AEN901` is not in `tan`'s own SDK-free
    fallback table at all, but the bound checkout's `metadata/e1m_modules`
    topology declares BOTH cores `edge-ai-starter`'s Alif tree needs -- the
    live SoM catalogue is the ground truth and overrides the static refusal."""
    sdk = _sdk_checkout(tmp_path / "sdk")
    modules = sdk / "metadata" / "e1m_modules"
    modules.mkdir(parents=True)
    (modules / "E1M-AEN901.yaml").write_text(
        "schema_version: 1\nsku: E1M-AEN901\ntopology:\n  a32_cluster: {}\n  m55_hp: {}\n",
        encoding="utf-8",
    )
    ...
    proc = run_tan(
        "init", "--template", "edge-ai-starter", "--som", "E1M-AEN901",
        "--sdk-root", str(sdk), "--destination", str(project), "--format", "json", cwd=tmp_path,
    )
    env = envelope(proc)

    assert proc.returncode == 0, env["issues"]
    assert "sku: E1M-AEN901" in (project / "board.yaml").read_text(encoding="utf-8")


def test_a_resolved_sdk_checkout_still_refuses_a_sku_its_own_topology_contradicts(tmp_path):
    """The override in the test above is not a blanket bypass: when the bound
    checkout's OWN topology proves the SKU genuinely lacks the core the tree
    needs, the refusal still fires."""
    sdk = _sdk_checkout(tmp_path / "sdk")
    modules = sdk / "metadata" / "e1m_modules"
    modules.mkdir(parents=True)
    (modules / "E1M-AEN901.yaml").write_text(
...
def test_a_resolved_sdk_checkout_never_overrides_iot_starters_silicon_gate(tmp_path):
    """`iot-starter`'s one-SKU gate is a silicon-validation fact (its CC3501E
    Wi-Fi bridge), not a core-topology one -- a SoM's topology proving it has
    the right CORES says nothing about whether its Wi-Fi transport has been
    bench validated, so the SDK-metadata override must never apply here, even
    with a checkout bound and even if that checkout's topology happens to
    match E1M-AEN801's own."""
    sdk = _sdk_checkout(tmp_path / "sdk")
    modules = sdk / "metadata" / "e1m_modules"
    modules.mkdir(parents=True)
    (modules / "E1M-AEN901.yaml").write_text(
    ...
    proc = run_tan(
        "init", "--template", "iot-starter", "--som", "E1M-AEN901",
        "--sdk-root", str(sdk), "--destination", str(project), "--format", "json", cwd=tmp_path,
    )
    env = envelope(proc)
