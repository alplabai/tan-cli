...
def test_ensure_writable_leaves_no_decoy_file_behind(tmp_path):
    """tan-cli#397: the writability probe opens for append, which CREATES the
    destination -- so a `--output` run had its file on disk before the emitter
    ran at all, and any "did the emit produce it?" check would pass for free on
    the exact path `cmake/alp.cmake` drives."""
    target = tmp_path / "generated" / "alp.conf"
    _ensure_writable(target, "zephyr-conf")
    assert target.parent.is_dir()
...
def test_an_output_override_that_never_gets_written_is_a_failed_target(
    tmp_path, monkeypatch, capsys
):
    """The `--output` path, which is what `cmake/alp.cmake` drives at configure
    time -- and the one an existence check alone cannot catch: `_ensure_writable`
    proves the destination writable by opening it for APPEND, creating it. So
    the run used to report a 0-byte file it made itself as the emitter's work,
    and CMake configured a whole build from it."""
    project = _project_for(tmp_path)
    sdk = _sdk_whose_emitter_writes_nothing(tmp_path)
    monkeypatch.chdir(project)
    monkeypatch.setattr(generate_cmd, "_planner_python", lambda *_a, **_k: sys.executable)
    ...
    assert env["data"]["failed"] == ["zephyr-conf"]
    assert env["issues"][0]["code"] == "generate.emit-failed"
    # No decoy left for the next reader (or the next run's check) either.
