...
`flash_method` on one helper. That combination is what alp-sdk's schema forbids
today (`{"required": ["update_channel"], "not": {"required": ["flash_method"]}}`
-- "an entry carries exactly one of the two"), and relaxing that XOR is the
upstream half of #611. tan is the CONSUMER, so its half has to be correct before
the upstream edit lands: an entry that declares a policy tan ignores is exactly
the silent-drop defect #611 reports, one field over.
...
def test_the_shipped_cc3501e_shape_is_untouched(tmp_path):
    """`update_channel` and no `flash_method` -- the CC3501E as it ships today.
    It still takes `_flash_entry`'s own branch and still gets that branch's
    wording. The hoist adds a decision above that guard; it does not reword the
    guard."""
