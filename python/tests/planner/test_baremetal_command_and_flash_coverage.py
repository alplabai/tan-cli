...
#: minus its final `add_compile_definitions(...)` line -- which is the only
#: part that varies with the slice. Split out so the two tests that assert
#: the file's contents can each pin the WHOLE file byte-for-byte without
#: either one restating five lines of banner.
...
def base_dir(tmp_path) -> Path:
    ...
    the helpers anchor every relative path on (issue #596). Resolved, so the
    `${PROJECT_ROOT}` tokenisation `_tokenize` performs against it matches
    the resolved paths `_resolve_app_path` / `_baremetal_output_dir` hand it.
    """
...
def test_a_baremetal_slice_flashes_via_the_cmake_recipe(base_dir):
    ...
    assert _slice_flash_recipe(_slice_of("baremetal", app="./src")) == (
...
def test_a_non_baremetal_slice_gets_a_different_flash_recipe(os_, expected):
    ...
    assert _slice_flash_recipe(_slice_of(os_, app="./src")) == expected
...
def test_a_non_baremetal_slice_does_not_get_a_cmake_configure(
    ...
    assert _slice_command(
        project, _slice_of(os_, **slice_kwargs), base_dir) == expected
