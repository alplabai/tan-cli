...
- **No absolute paths in argv** — every case invokes `tan` without
  ...
  uses) as a result.
- **Path-separator normalization** — the one thing case selection can't avoid
  by construction: `PathBuf::to_string_lossy()` renders `./board.yaml` as
  `.\board.yaml` on Windows. The harness recursively normalizes every JSON
  string leaf (`\` → `/`) on the freshly captured side before diffing; the
  committed goldens are authored with forward slashes, matching that
  normalized form.
...
|---|---|---|---|
| `init-preview-minimal-app` | `init --template minimal-app --preview --format json` | 0 | Deterministic scaffold plan — the shape `generate`/`init` templates hand the extension. |
