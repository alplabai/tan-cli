...
| `data.availablePorts[].{device,description}` | `monitor` | golden `monitor-no-port` (tan-cli#1165). Read by alp-sdk-vscode `src/monitor.ts::listSerialPorts`; a rename of either key makes the extension silently report "No serial ports were found" while `tan monitor` itself printed a full list. Pins one entry with a real `description` and one with the empty string `_ports_data` emits when pyserial reports none -- the consumer's own fallback rendering ("n/a") is not a fact this envelope carries, only the empty string is. `env.json`-armed (see "Pinning host state a case reads" below): `_available_ports()`'s real source enumerates whatever serial hardware is attached to the recording machine, which is a fact about that machine, not the contract. |
...
- **Pinning host state a case reads (`env.json`, tan-cli#253)** — every rule
  ...
  **`monitor-no-port` is the third, tan-cli#1165.** `data.availablePorts` is
  ...
  The other twenty-three cases answer from their own copied inputs and carry
...
| `monitor-no-port` | `monitor --format json` (no `--port`, `env.json`) | 1 | tan-cli#1165. Pins `data.availablePorts[].{device,description}` — read by alp-sdk-vscode `src/monitor.ts::listSerialPorts` and otherwise unguarded by any golden — with two entries: one carrying a real `description`, one carrying the empty string `_ports_data` emits when pyserial reports none. Also pins the `monitor.no-port` issue and its "no --port given -- available serial ports: ..." message. The one case in this set whose command reads live serial hardware, hence the `env.json`; see "Pinning host state a case reads" below. |
