...
| `model-unknown-subcommand` | `model bogus --format json` | 1 | tan-cli#253; the direct analogue of `sdk-unknown-subcommand`. Also pins the shipped subcommand inventory, since the message's "Available: build, doctor, check, list." is rendered from `SUBCOMMANDS` itself. Re-recorded for tan-cli#674 when `list` landed as a fourth subcommand (`PROVENANCE.txt`); only the trailing inventory moved. |
