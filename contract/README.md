...
| `data.sku`, `.boardYaml`, `.slices[].{coreId,backend,buildDir,env,command,configArtefacts[].{path,contents}}`, `.sharedArtefacts[].{path,contents}`, `.warnings[].{code,coreId,message}` | `build --plan` | **NOT COVERED, and unreachable.** The flag refuses today: `tan build --plan --format json` answers `ok:false`, `exitCode:1`, `cli.command-deferred` (tan-cli#427), so the command emits no `data` at all and there is nothing yet to freeze. When #427 lands it inherits `data.written`'s blocker too (a resolvable alp-sdk checkout + a Python spawn). Read by `src/ideHub/buildPlanPanel.ts` (tan-cli#200). |
