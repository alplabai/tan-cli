...
| `validate-offline-som-scalar-leading-zero` | `validate --offline --format json` (fixture `board.yaml`: `som: 010`) | 2 | Pins `validate.rs`'s `python_repr` output for a `som:` scalar whose type resolution is a known divergence between serde_yaml's YAML 1.2 core schema (plain string `'010'`) and PyYAML's YAML 1.1 resolver (octal int `8`) — a regression gate on the CURRENT Rust output, not a Python-parity claim for this input. |
| `validate-offline-som-scalar-yes` | `validate --offline --format json` (fixture `board.yaml`: `som: yes`) | 2 | Same divergence, the boolean-resolution case: serde_yaml keeps `'yes'` a string; PyYAML's YAML 1.1 resolver reads it as `True`. |
