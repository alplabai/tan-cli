...
| `data.schemaVersion`, `.planPathMode`, `.sku`, `.boardYaml`, `.slices[].{coreId,backend,buildDir,env,envAppendPath,command,configArtefacts[].{path,contents}}`, `.sharedArtefacts[].{path,contents}`, `.warnings[].{code,coreId,message}` | `build --plan` | **NOT COVERED, and unreachable.** The flag refuses today: `tan build --plan --format json` answers `ok:false`, `exitCode:1`, `cli.command-deferred` (tan-cli#427), so there is no plan `data` to freeze yet -- the refusal envelope's `data` carries only the deferral `message`. Two different producers sit behind this row, and only one of them is this one (tan-cli#853): `--plan-from` (reachable today) reads the caller's own plan FILE and echoes it verbatim at exit 0 (`_acquire_plan`/`_MODE_PLAN`, `python/tan/commands/build_cmd.py:1487-1494`, which returns BEFORE `apply_plan_token_substitution` runs at `:1502`) -- a passthrough of the caller's file, not this emitter, so a golden recorded from `--plan-from` freezes nothing about this row's shape no matter what plan it is fed. `--plan` (once #427 lands) instead echoes whatever `emit_build_plan` (`python/tan/planner/buildplan.py:375`) renders in-process -- MEASURED against a real board (`emit('build-plan', ..., board_yaml=examples/multicore/rpmsg-v2n/board.yaml)`): the plan is tagged `"planPathMode": "tokened"` and every slice's `env.ALP_SDK_ROOT` / `envAppendPath.{EXTRA_ZEPHYR_MODULES,PYTHONPATH}` carries a literal, UNSUBSTITUTED `${SDK_ROOT}` token (`buildplan.py:610,620-623,636`) -- so a `--plan-from` golden pins whatever fixture it was handed, not this emitter: `--plan-from` echoes the caller's file verbatim, tokens or not (`_MODE_PLAN`, `build_cmd.py:1487-1494`), so whether it shows `${SDK_ROOT}` depends entirely on the plan it was fed, not on anything `--plan-from` itself guarantees. The blocker is also re-measured rather than inherited: planning is IN-PROCESS (`_emit_plan`, `build_cmd.py:734`, "no interpreter on PATH, no PYTHONPATH, no process, and nothing under `<sdk>/scripts` imported or spawned") and `_MODE_PLAN` returns before token substitution -- the one place a Python interpreter gets resolved -- ever runs; there is no Python *interpreter* spawn on this path, but not "no process" full stop -- `emit_build_plan` calls `_sdk_commit()` (`buildplan.py:339-372`), which spawns a `git rev-parse --short HEAD` subprocess on every plan (`buildplan.py:365-366`); the docstring's "no process" is true of `<sdk>/scripts` and the interpreter, not of processes generally. The two real blockers are two of `_emit_plan`'s seven `build.plan-unavailable` refusal sites (`build_cmd.py:734-833`): a resolvable alp-sdk checkout (`build_cmd.py:768`) and a `board.yaml` (`build_cmd.py:776`) -- the other five (an incomplete checkout missing `board.schema.json` at `:788`, and four planner-import/exec failures at `:798`, `:811`, `:822`, `:831`) also refuse before any `data` exists. Not the Python spawn `data.written`'s row names for `--materialise`. Read by `src/ideHub/buildPlanPanel.ts` (tan-cli#200). |
